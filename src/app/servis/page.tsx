...
import { Loader2, Search, Database, X, Plus, Trash2 } from 'lucide-react';
...
export default function ServisPage() {
    ...
    const fetchProjects = useCallback(async () => {
        ...
        // Load Global Import Metadata from DB (Service Specific)
    ...
    // Universal filter – searches across ALL fields
                ...
                {isLoading ? (
                    <div className="flex items-center justify-center h-64 text-muted-foreground">
                        <div className="flex flex-col items-center gap-3">
                            ...
                            <span className="text-[9px] font-bold uppercase tracking-widest opacity-50">Načítám servisy...</span>
                        </div>
                    </div>
                ) : (
                    <DataTable
                        ...
                        leftToolbar={
                            <div className="flex items-center gap-3 w-full">
                                ...
                                <div className="hidden md:flex items-center gap-1" title="Zobrazeno / Celkem servisů">
                                    <span
                                        className={cn(
                                            ...
                                            "bg-[#1a1a1a] text-orange-500 border-orange-500/20 shadow-orange-500/10"
                                        )}
                                    >
                                ...
                                <div className="relative group max-w-xs flex-1" title="Hledat lze podle: Názvu, ID, Zákazníka, Manažera, Stavu, Výrobního čísla, Abra kódů a dalších vlastních polí.">
                                    <div className="flex items-center gap-1.5 w-full min-h-[30px] px-2.5 py-0.5 bg-muted/20 border border-border/60 rounded-lg focus-within:ring-1 focus-within:ring-primary/20 focus-within:border-primary/40 transition-all flex-wrap">
                                        ...
                                        <input
                                            type="text"
                                            placeholder={searchTags.length === 0 ? "Hledat servis..." : ""}
                        ...
                        onRowClick={(row) => router.push(`/servis/${row.id}`)}
                        ...
                        headerClassName={'bg-orange-100 text-orange-900'}
                        getRowClassName={(row: Project) => {
                            return 'bg-orange-500/5 hover:bg-orange-500/10 active:bg-orange-500/20';
                        }}
                    />
                )}
            </div>

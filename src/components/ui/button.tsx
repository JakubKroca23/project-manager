...
// Since Radix is not installed, we can remove the Slot dependency or mock it if we wanted strict compatibility, 
// but for now I will just use standard button/comp props. 
// Actually I will skip Slot for now as I don't see it in package.json.
  ...
  "inline-flex items-center justify-center whitespace-nowrap rounded-xl text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 transition-all duration-200 active:scale-95",
  {
    variants: {
      variant: {
        ...
        premium: "bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 hover:from-indigo-600 hover:to-purple-700 border-0"
      },
      size: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    // Simplified without Slot for now as Radix is not present
    const Comp = "button"
    return (
      <Comp
